...
class EvolutionaryAlgorithm:
    ...
    def NSGA(self, R, P_t, A_t, T_t1, t_past, t_freq, t, n, jutting_param, mse, model_performance, rf_params=None):
        self.history_P.append(P_t)
        T_t = update_target_archive(P_t, T_t1, R)
        count = t % t_freq
        if count == 0:
            D_t = archive_mapping(A_t, T_t, R)
            predict, x_min, x_max = training(D_t, self.problem.xl, self.problem.xu, mse, model_performance, rf_params=rf_params)
